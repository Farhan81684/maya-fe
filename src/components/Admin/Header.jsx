    ...
    try {
      ...
      if (res?.status === 200) {
        ...
        localStorage.setItem(
          "user",
          JSON.stringify({ ...user, name: values?.name })
        );
      ...
      <ImageUploading
        ...
        onError={(error) => {
          if (error?.maxFileSize)
            return message?.error("Image cannot be greater than 20mb");
          if (error?.acceptType)
            return message?.error("Images allowed are jpg, png, jpeg");
        }}
      >
        {({
          imageList,
                ...
                <Image
                  ...
                  src={
                    user?.profile_pic_url
                      ? `http://192.168.18.24:4006${user?.profile_pic_url}`
                      : dummy
                  }
                  alt="Profile"
                  width={56}
