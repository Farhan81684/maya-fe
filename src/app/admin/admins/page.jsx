...
const AdminsPage = () => {
  ...
  const paginatedData = admins?.length
    ? admins?.slice(
    ...
    try {
      if (uploadedImage[0]?.file) {
        ...
        let response = await axios
          .post(
            `${process.env.NEXT_PUBLIC_API_URL}/util/upload?route_id=${1}&id=${
              selectedAdmin?.id
            }`,
            formData,
            {
